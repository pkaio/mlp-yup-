...
router.get('/admin/stats', authMiddleware, requireSuperModerator, async (_req, res) => {
  try {
    ...
    res.json({
      totalVideos: pendingVideos.rows[0]?.count ?? 0,
      totalUsers: activeUsers.rows[0]?.count ?? 0,
      totalParks: 0, // We'll fetch this separately
      xpThisWeek: xpWeek.rows[0]?.total ?? 0,
